...
namespace ap {
	...
	private:
		...
		bool is_multi_shortarg(const std::string& s) const {
	...
	public:
		...
		bool add(const std::string& shortarg,
			...
			bool booltype = (m == ap::E_BOOLEAN);
			bool required = (m == ap::E_REQUIRED);

			m_args.push_back(argstruct(shortarg, longarg, helpstr, booltype, required, false));
